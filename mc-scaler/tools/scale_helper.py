...
def scale_with_helper_host(client: Client, helper_type: ServerType, city: str, scale_host_name: str,
                           scale_type: ServerType):
    helper_host_name = "{}-helper".format(scale_host_name)

    logging.info("Start scaling via an helper (%s,%s). Target host '%s' scaling to %s", helper_host_name,
                 helper_type.name,
                 scale_host_name, scale_type.name)
